import torch
import torch.nn as nn
import torch.nn.functional as F
import cv2
import numpy as np
import os
import glob
from skimage.morphology import binary_dilation, disk
import argparse

import trimesh
from pathlib import Path
import subprocess

import sys
import render_utils as rend_util
from tqdm import tqdm


def _sorted_pngs(directory):
    return sorted(
        path for path in glob.glob(os.path.join(directory, "*.png"))
        if not os.path.basename(path).startswith("._")
    )


def _path_stem_index(path):
    try:
        return int(Path(path).stem)
    except ValueError:
        return None


def _match_mask_paths(image_paths, mask_paths, mask_dir):
    if len(mask_paths) == len(image_paths):
        return mask_paths

    mask_by_index = {}
    for mask_path in mask_paths:
        mask_index = _path_stem_index(mask_path)
        if mask_index is not None:
            mask_by_index[mask_index] = mask_path

    matched_mask_paths = []
    for image_path in image_paths:
        image_index = _path_stem_index(image_path)
        if image_index is None or image_index not in mask_by_index:
            raise ValueError(
                f"Expected {len(image_paths)} masks in {mask_dir}, but found {len(mask_paths)} valid PNG files."
            )
        matched_mask_paths.append(mask_by_index[image_index])

    return matched_mask_paths


def resolve_mask_dir(mask_arg, scene_dir, scan_id):
    if mask_arg is None:
        return os.path.join(scene_dir, "mask")

    direct_mask_dir = Path(mask_arg)
    if direct_mask_dir.name == "mask" and direct_mask_dir.is_dir():
        return str(direct_mask_dir)

    candidate_dirs = [
        direct_mask_dir / f"scan{scan_id}" / "mask",
        direct_mask_dir / "mask",
    ]
    for candidate in candidate_dirs:
        if candidate.is_dir():
            return str(candidate)

    return str(direct_mask_dir)


def cull_scan(scan, mesh_path, result_mesh_file, scene_dir, mask_dir=None):
    mask_dir = resolve_mask_dir(mask_dir, scene_dir, scan)

    # load poses
    image_dir = '{0}/images'.format(scene_dir)
    image_paths = _sorted_pngs(image_dir)
    # print(f"image_paths is {image_paths}")
    n_images = len(image_paths)
    cam_file = '{0}/cameras.npz'.format(scene_dir)
    camera_dict = np.load(cam_file)
    scale_mats = [camera_dict['scale_mat_%d' % idx].astype(np.float32) for idx in range(n_images)]
    world_mats = [camera_dict['world_mat_%d' % idx].astype(np.float32) for idx in range(n_images)]

    intrinsics_all = []
    pose_all = []
    for scale_mat, world_mat in zip(scale_mats, world_mats):
        P = world_mat @ scale_mat
        P = P[:3, :4]
        intrinsics, pose = rend_util.load_K_Rt_from_P(None, P)
        intrinsics_all.append(torch.from_numpy(intrinsics).float())
        pose_all.append(torch.from_numpy(pose).float())
    
    # load mask
    mask_paths = _sorted_pngs(mask_dir)
    # print(f"n_images is {n_images}")
    # print(f"mask_paths is {mask_paths}")
    mask_paths = _match_mask_paths(image_paths, mask_paths, mask_dir)
    masks = []
    for p in mask_paths:
        mask = cv2.imread(p)
        if mask is None:
            raise ValueError(f"Failed to read mask image: {p}")
        masks.append(mask)

    # hard-coded image shape
    W, H = 1600, 1200

    # load mesh
    mesh = trimesh.load(mesh_path)
    
    # load transformation matrix

    vertices = mesh.vertices

    # project and filter
    vertices = torch.from_numpy(vertices).cuda()
    vertices = torch.cat((vertices, torch.ones_like(vertices[:, :1])), dim=-1)
    vertices = vertices.permute(1, 0)
    vertices = vertices.float()

    sampled_masks = []
    for i in tqdm(range(n_images),  desc="Culling mesh given masks"):
        pose = pose_all[i]
        w2c = torch.inverse(pose).cuda()
        intrinsic = intrinsics_all[i].cuda()

        with torch.no_grad():
            # transform and project
            cam_points = intrinsic @ w2c @ vertices
            pix_coords = cam_points[:2, :] / (cam_points[2, :].unsqueeze(0) + 1e-6)
            pix_coords = pix_coords.permute(1, 0)
            pix_coords[..., 0] /= W - 1
            pix_coords[..., 1] /= H - 1
            pix_coords = (pix_coords - 0.5) * 2
            valid = ((pix_coords > -1. ) & (pix_coords < 1.)).all(dim=-1).float()
            
            # dialate mask similar to unisurf
            maski = masks[i][:, :, 0].astype(np.float32) / 256.
            maski = torch.from_numpy(binary_dilation(maski, disk(24))).float()[None, None].cuda()

            sampled_mask = F.grid_sample(maski, pix_coords[None, None], mode='nearest', padding_mode='zeros', align_corners=True)[0, -1, 0]

            sampled_mask = sampled_mask + (1. - valid)
            sampled_masks.append(sampled_mask)

    sampled_masks = torch.stack(sampled_masks, -1)
    # filter
    
    mask = (sampled_masks > 0.).all(dim=-1).cpu().numpy()
    face_mask = mask[mesh.faces].all(axis=1)

    mesh.update_vertices(mask)
    mesh.update_faces(face_mask)
    
    # transform vertices to world 
    scale_mat = scale_mats[0]
    mesh.vertices = mesh.vertices * scale_mat[0, 0] + scale_mat[:3, 3][None]
    mesh.export(result_mesh_file)
    del mesh
    

if __name__ == "__main__":

    parser = argparse.ArgumentParser(
        description='Arguments to evaluate the mesh.'
    )

    parser.add_argument('--input_mesh', type=str,  help='path to the mesh to be evaluated')
    parser.add_argument('--scan_id', type=str,  help='scan id of the input mesh')
    parser.add_argument('--output_dir', type=str, default='evaluation_results_single', help='path to the output folder')
    parser.add_argument('--scene_dir', type=str, default=None, help='path to the DTU scan directory that contains images and cameras.npz')
    parser.add_argument('--mask_dir', type=str, default=None, help='path to masks, a scene root, or a dataset root containing scanXX/mask')
    parser.add_argument('--DTU', type=str,  default='Offical_DTU_Dataset', help='path to the GT DTU point clouds')
    args = parser.parse_args()

    Offical_DTU_Dataset = args.DTU
    out_dir = args.output_dir
    Path(out_dir).mkdir(parents=True, exist_ok=True)

    scan = args.scan_id
    ply_file = args.input_mesh
    print("cull mesh ....")
    result_mesh_file = os.path.join(out_dir, "culled_mesh.ply")
    if args.scene_dir is not None:
        scene_dir = args.scene_dir
        mask_dir = args.mask_dir
    else:
        scene_dir = os.path.join(args.mask_dir, f'scan{args.scan_id}')
        mask_dir = None
    cull_scan(scan, ply_file, result_mesh_file, scene_dir=scene_dir, mask_dir=mask_dir)

    script_dir = os.path.dirname(os.path.abspath(__file__))
    cmd = f"python {script_dir}/eval.py --data {result_mesh_file} --scan {scan} --mode mesh --dataset_dir {Offical_DTU_Dataset} --vis_out_dir {out_dir}"
    os.system(cmd)
